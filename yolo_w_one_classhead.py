...
class UnifiedHead(nn.Module):
    def __init__(self, input_channels, output_dims):
        """
        input_channels: 입력 채널 수
        output_dims: 속성별 출력 크기 (리스트)
        """
        super(UnifiedHead, self).__init__()
        c_ = 1280  # efficientnet_b0 size
        self.conv = Conv(input_channels, c_, k=1, s=1, p=None, g=1)
        self.pool = nn.AdaptiveAvgPool2d(1)
        self.drop = nn.Dropout(p=0.0, inplace=True)
        self.linear = nn.Linear(c_, sum(output_dims))  # 모든 속성 출력
    ...
    def forward(self, x):
        ...
        for i, dim in enumerate(self.output_dims):
            outputs[f"attribute_{i+1}"] = x[:, start_idx:start_idx + dim]
            start_idx += dim
        return outputs
...
if __name__ == '__main__':
    # 각 속성의 출력 크기 설정
    output_dims = [
        len(category_encodings["기장"]), 
        len(category_encodings["색상"]),
        len(category_encodings["소매기장"]),
        len(category_encodings["소재"]),
        len(category_encodings["프린트"]),
        len(category_encodings["넥라인"]),
        len(category_encodings["핏"])
    ]
